...
void ImGui::TableSetColumnWidth(int column_n, float full_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ...
    float column_0_width = full_width;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    const float full_width = preserve_width ? GetColumnWidthEx(columns, column_index, columns->IsBeingResized) : 0.0f;
    ...
    if (preserve_width)
        SetColumnOffset(column_index + 1, offset + ImMax(g.Style.ColumnsMinSpacing, full_width));
}

void ImGui::SetColumnWidth(int column_index, float full_width)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
    IM_ASSERT(columns != NULL);
    ...
    if (column_index < 0)
        column_index = columns->Current;
    SetColumnOffset(column_index + 1, GetColumnOffset(column_index) + full_width);
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    float full_width = offset_1 - offset_0;
    PushItemWidth(full_width * 0.65f);
