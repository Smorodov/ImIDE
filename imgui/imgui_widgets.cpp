...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size = ImMax(bars_width * 1, full_width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
...
void ImGuiMenuColumns::CalcNextTotalWidth(bool update_offsets)
{
    ...
    {
        ImU16 full_width = Widths[i];
        if (want_spacing && full_width > 0)
            offset += Spacing;
        want_spacing |= (full_width > 0);
        if (update_offsets)
        {
...
bool ImGui::BeginMainMenuBar()
{
    ...
    float full_height = GetFrameHeight();
    bool is_open = BeginViewportSideBar("##MainMenuBar", viewport, ImGuiDir_Up, full_height, window_flags);
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    ImGuiContext& g = *GImGui;
    const float full_width = bb.GetWidth();
    IM_UNUSED(flags);
    IM_ASSERT(full_width > 0.0f);
    const float rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, full_width * 0.5f - 1.0f));
