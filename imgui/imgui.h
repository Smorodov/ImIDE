...
namespace ImGui
{
    ...
    IMGUI_API void          SetColumnWidth(int column_index, float full_width);                      // set column width (in pixels). pass -1 to use current column
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      Draw(const char* label = "Filter (inc,-exc)", float full_width = 0.0f);  // Helper calling InputText+Build
...
struct ImFontAtlas
{
    ...
    IMGUI_API int               AddCustomRectRegular(int full_width, int full_height);
    IMGUI_API int               AddCustomRectFontGlyph(ImFont* font, ImWchar id, int full_width, int full_height, float advance_x, const ImVec2& offset = ImVec2(0, 0));
