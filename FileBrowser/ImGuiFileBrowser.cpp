...
namespace imgui_addons
{
    ImGuiFileBrowser::ImGuiFileBrowser()
    {
        ...
        invfile_modal_id = (const char*)u8"Неправильный файл!";
        repfile_modal_id = (const char*)u8"Заменить файл?";
        selected_fn = "";
        selected_path = "";
        input_fn[0] = '\0';
        ...
        if (ImGui::BeginPopupModal(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            if(validate_file)
            {
                ...
                else if(!check && dialog_mode == DialogMode::SELECT)
                {
                    ...
                    selected_path.clear();
                    show_error = true;
                    error_title = (const char*)u8"Неправильная директория!";
                    error_msg =(const char*)u8"Выбрана неправильная директория. Убедитесь что директория существует.";
                }
    ...
    bool ImGuiFileBrowser::renderNavAndSearchBarRegion()
    {
        ...
        for(int i = 0; i < current_dirlist.size(); i++)
        {
            ...
            {
                ImGui::SameLine(0,0);
                float next_label_width = (float)ImGui::CalcTextSize(current_dirlist[i+1].c_str()).x;
        ...
        ImGui::SameLine();
        showHelpMarker((const char*)u8"Фильтр (вкл, -искл)");

        ImGui::EndChild();
        return show_error;
    }
    // --------------------------
    // Отрисовка файловой панели
    // ---------------------------
    bool ImGuiFileBrowser::renderFileListRegion()
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.5, 1, 0.5,1.0f));
        int items = 0;
    ...
    {
        std::string label = (dialog_mode == DialogMode::SAVE) ? (const char*)u8"Сохранить как:" : (const char*)u8"Открыть:";
        ImGuiStyle& style = ImGui::GetStyle();
        ImGuiIO& io = ImGui::GetIO();
        ...
        if(ext_box_width < 0.0)
            ext_box_width = ImGui::CalcTextSize(".abcz").x + 100;
        ...
        if(ImGui::InputTextWithHint("##FileNameInput", (const char*)u8"имя.tbl", &input_fn[0], 256, ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_AutoSelectAll))
        {
            if ( strlen( input_fn ) > 0 )
            {
                if ( fs::is_directory(input_fn))
                {
    ...
    bool ImGuiFileBrowser::renderButtonsAndCheckboxRegion()
    {
        ...
        float frame_height = ImGui::GetFrameHeight();
        float frame_height_spacing = ImGui::GetFrameHeightWithSpacing();
        float opensave_btn_width = getButtonSize((const char*)u8"Открыть").x;     // Since both Open/Save are 4 characters long, width gonna be same.
        float selcan_btn_width = getButtonSize((const char*)u8"Отмена").x;     // Since both Cacnel/Select have same number of characters, so same width.
        float buttons_xpos;
        ...
        /*
        //Render Checkbox
        float label_width = ImGui::CalcTextSize((const char*)u8"Показывать скрытые файлы и папки").x + ImGui::GetCursorPosX() + frame_height;
        bool show_marker = (label_width >= buttons_xpos);
        ImGui::Checkbox( (show_marker) ? "##showHiddenFiles" : u8"Показывать скрытые файлы и папки", &show_hidden);
        if(show_marker)
        {
            ImGui::SameLine();
            showHelpMarker((const char*)u8"Показывать скрытые файлы и папки");
        }
       ...
       ImGui::SetCursorPosX(buttons_xpos);
        if(dialog_mode == DialogMode::SAVE)
        {
            ...
            {
                if (ImGui::Button((const char*)u8"Открыть"))
                    show_error |= !(onDirClick(selected_idx));
            }
            else if (ImGui::Button((const char*)u8"Сохранить") && strlen(input_fn) > 0)
            {
                selected_fn = std::string(input_fn);
                validate_file = true;
            }
        ...
        else
        {
            ...
            if(dialog_mode == DialogMode::SELECT)
            {
                ...
                ImGui::SameLine();
                if (ImGui::Button((const char*)u8"Выбрать"))
                {
                    if(strlen(input_fn) > 0)
                    {
                        selected_fn = std::string(input_fn);
        ...
        ImGui::SameLine();
        if (ImGui::Button((const char*)u8"Отмена"))
            closeDialog();
            ...
            if(ImGui::ListBoxHeader("##InputBarComboBoxList", listbox_size))
            {
                ...
                for(auto& element : inputcb_filter_files)
                {
                    if(ImGui::Selectable(element.get().c_str(), false, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick))
                    {
                        ...
                        {
                            error_title = (const char*)u8"Ошибка!";
                            error_msg =(const char*) u8"Выбранное имя файла длиннее чем 255 символоа.";
                            show_error = true;
                        }
                        else
                        {
    ...
    void ImGuiFileBrowser::renderExtBox()
    {
        ...
        if ( show_files_with_valid_extensions )
            selected_label = (const char*)u8".tbl";
        if ( show_all_files )
            selected_label = (const char*)u8"Все (*.*)";

        if(ImGui::BeginCombo("##FileTypes", selected_label ))
        {
            /*
            if ( ImGui::Selectable((const char*)u8".tbl", &show_files_with_valid_extensions ) )
            {
                show_all_files = false;
            ...
            */
            ...
            if ( ImGui::Selectable( (const char*)u8"Все (*.*)", &show_all_files ) )
            {
                show_files_with_valid_extensions = false;
                filterFiles( FilterMode_Files );
            }
    ...
    bool ImGuiFileBrowser::onNavigationButtonClick(int idx)
    {
        ...
        if(idx == 0)
        {
            ...
            current_path.clear();
            current_dirlist.clear();
            current_dirlist.push_back((const char*)u8"Компьютер");
            return true;
            #else
            new_path = "/";
            #endif // OSWIN
    ...
    bool ImGuiFileBrowser::onDirClick(int idx)
    {
        ...
        #ifdef OSWIN
        drives_shown = (current_dirlist.size() == 1 && current_dirlist.back() == (const char*)u8"Компьютер");
        #endif // OSWIN
...
bool ImGuiFileBrowser::readDIR(std::string pathdir)
    {  
        // --------------------------
        // --------------------------
        std::string abs_path=fs::absolute(pathdir).string();
        current_path=abs_path;
        std::replace(abs_path.begin(), abs_path.end(), '\\', '/');
        parsePathTabs(abs_path);
            std::string path = fs::absolute(current_path).string();
            if(path=="")
            {
                return false;
            }
            ...
            for (const auto & entry : fs::directory_iterator(path))
            {
                if(entry.is_directory())
                {
                     ...
                     size_t currentPosition = filePath.find_last_of(entry.path().preferred_separator);
                     if(currentPosition!= std::string::npos)
                     {
                        res=filePath.substr(currentPosition+1);
                     }
                     std::cout << "[D]" << entry.path() << std::endl;
                    subdirs.push_back(Info(res, false));                
                ...
                if(entry.is_regular_file() )
                {
                    ...
                    subfiles.push_back(Info(entry.path().filename().string(), false));
                }                
            }
            // ---------------------
            // ---------------------
      ...
      //  else
      //  {
      //      error_title = u8"Ошибка!";
      //      error_msg = u8"Ошибка открытия директории! Убедитесь что директория существует м вы имеете права доступа к ней.";
      //      return false;
      //  }
    ...
    bool ImGuiFileBrowser::readDIR(std::string pathdir)
    {
        ...
        if (dir != nullptr)
        {
            ...
            if(current_dirlist.empty() && pathdir == "./")
            {
                ...
                std::string current_directory = wstring_to_utf8(absolute_path);
                std::replace(current_directory.begin(), current_directory.end(), u8'\\', u8'/');
            ...
            for (const auto & entry : fs::directory_iterator(path))
            {
                ...
                if(entry.is_regular_file() )
                {
                    std::wcout << "[F]" << entry.path() << std::endl;
                }
...
*/
    ...
    bool ImGuiFileBrowser::showReplaceFileModal()
    {
        ...
        if (ImGui::BeginPopupModal(repfile_modal_id.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize))
        {
            ...
            std::string text =(const char*) u8"Файл с таким именем существует. Заменить его?";
            ImGui::TextWrapped("%s", text.c_str());

            ImGui::Separator();

            float buttons_width = getButtonSize((const char*)u8"Да").x + getButtonSize((const char*)u8"Нет").x + ImGui::GetStyle().ItemSpacing.x;
            ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetWindowWidth()/2.0f - buttons_width/2.0f - ImGui::GetStyle().WindowPadding.x);

            if (ImGui::Button((const char*)u8"Да", getButtonSize((const char*)u8"Да")))
            {
                selected_path = current_path + selected_fn;
                ImGui::CloseCurrentPopup();
                ret_val = true;
            ...
            ImGui::SameLine();
            if (ImGui::Button((const char*)u8"Нет", getButtonSize((const char*)u8"Нет")))
            {
                selected_fn.clear();
                selected_path.clear();
                ImGui::CloseCurrentPopup();
    ...
    {
        ImGuiStyle& style = ImGui::GetStyle();
        std::string text = (const char*)u8"Этот файл не существует или не поддерживается. Выберите файл с расштрением из списка ...";
    ...
    bool ImGuiFileBrowser::loadWindowsDrives()
    {
        ...
        for(char *drv = nullptr; *temp != '\0'; temp++)
        {
            ...
            if(DRIVE_REMOVABLE == GetDriveTypeA(drv))
                subdirs.push_back({(const char*)u8"Съемный диск: " + std::string(1,drv[0]), false});
            else if(DRIVE_FIXED == GetDriveTypeA(drv))
                subdirs.push_back({(const char*)u8"Диск: " + std::string(1,drv[0]), false});
